    ...
    internal static class AssetCache
    {
        ...
        public static DataTable GetAssetTable(IList<WhereClause> clauses)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(m_connectionString))
            {
                ...
                sql = new StringBuilder();
                sql.Append("SELECT ");
                sql.Append("a.*, t.typeName, t.basePrice, t.portionSize, g.groupName, cat.categoryName, f.flagName, ct.typeName || ' #' || c.itemID AS containerName, c.typeID AS containerTypeID, cg.groupName AS containerGroup, ccat.categoryName AS containerCategory, mlattr.valueInt AS metaLevel ");
                if (gotOutposts)
